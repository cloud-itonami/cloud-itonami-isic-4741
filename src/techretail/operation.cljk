(ns techretail.operation
  "OperationActor -- one computer-retail/trade-in operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Retail Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Retail Governor (:govern) and
  ...
  One graph run = one computer-retail/trade-in operation (intake ->
  ...
  decision to a human operator (the retail-operations approver). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/fulfill-order`/`:actuation/issue-
  sanitization-certificate` ALWAYS reach this node when the governor is
  clean -- see `techretail.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [techretail.techretailadvisor :as techretailadvisor]
            [techretail.governor :as governor]
...
(defn build
  ...
  `techretail.store/Store`).
  opts:
    :advisor      -- a `techretail.techretailadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (techretailadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Retail Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (techretailadvisor/-advise advisor store request)]
            {:proposal p :audit [(techretailadvisor/trace request p)]})))

      ;; Retail Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
